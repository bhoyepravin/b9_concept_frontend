...
const VideoHeroSection = () => {
  ...
  // Auto-slide videos when video ends
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      // If video is near the end (last 0.5 seconds), setup fallback
  ...
  const handleVideoError = () => {
    setVideoError(true);
    // If video fails to load, move to next slide after a delay
    if (isPlaying) {
      setTimeout(() => {
        setDirection(1);
        setCurrentVideoIndex(
  ...
  return (
    <section className="relative w-full md:h-screen h-80 overflow-hidden">
      ...
      <div className="relative z-20 w-full h-full flex flex-col items-center justify-center text-white text-center px-4">
        ...
        {/* Navigation Arrows */}
        <motion.button
          className="absolute left-4 top-1/2 mt-14 transform -translate-y-1/2 bg-white/20 hover:bg-white/30 rounded-full p-3 transition-all duration-300 z-30  backdrop-blur-sm"
          onClick={() =>
            goToSlide(
        ...
        <motion.button
          className="absolute right-4 top-1/2 mt-14 transform -translate-y-1/2 bg-white/20 hover:bg-white/30 rounded-full p-3 transition-all duration-300 z-30 backdrop-blur-sm"
          onClick={() =>
            goToSlide((currentVideoIndex + 1) % heroVideosData.videos.length)
          }
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
        ...
        {isVideoPlaying && videoSource && !videoError && (
          <motion.div
            className="absolute bottom-12 left-1/2 transform -translate-x-1/2 w-48 h-1 bg-white/30 rounded-full overflow-hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            <motion.div
              ...
              animate={{ width: "100%" }}
              transition={{
                duration: currentVideo.duration || 10, // Use video duration if available
                ease: "linear",
              }}
            />
          </motion.div>
